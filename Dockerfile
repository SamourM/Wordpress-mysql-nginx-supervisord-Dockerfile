...
RUN apt -y install software-properties-common wget curl  nginx  mysql-server supervisor curl php-fpm php-common php-mbstring php-xmlrpc php-soap php-gd php-xml php-intl php-mysql php-cli php-ldap php-zip php-curl #install php-fpm
EXPOSE 3306 80
...
mysql -u root -p  -e "CREATE DATABASE wordpress;CREATE USER 'wordpressuser'@'localhost' IDENTIFIED BY 'test';GRANT ALL ON wordpress.* TO 'wordpressuser'@'localhost' IDENTIFIED BY 'test' WITH GRANT OPTION;FLUSH PRIVILEGES;"
